...
# data = pd.read_csv('data/BankFAQs.csv')
...
def cleanup(sentence):
    word_tok = sentence.split()  # Simple tokenization using split
    stemmed_words = [w for w in word_tok if not w in stop_words]
    return ' '.join(stemmed_words)
...
def get_response(usrText):
    while True:
        ...
        cos_sims = [cosine_similarity(tfv.transform([question]), t_usr) for question in questionset['Question']]
        ind = cos_sims.index(max(cos_sims))
...
def get_response2(usr):
    ...
    if max(cos_sims) == [[0.]]:
        return "I'm not able to solve this question at this moment. You can call customer support 1860 999 9999 \U0001F615"
